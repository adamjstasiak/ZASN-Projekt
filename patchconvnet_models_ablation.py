...
class PatchConvnet(nn.Module):
    def __init__(
        ...
        hybrid_backbone: Optional[bool] = None,
        norm_layer=nn.LayerNorm,
...
class PatchConvNetAblation(nn.Module):
    def __init__(self,
        ...
        self.model = S60(pretrained=pretrained,num_classes=num_classes)
        if use_relu:
            self._replace_attention_with_gap()
        if use_gap:
            self._replace_gelu_relu()
    ...
    def _replace_attention_with_gap(self):
        if hasattr(self.model, 'global_pool'):
            self.model.global_pool = nn.AdaptiveAvgPool1d(1)
        if hasattr(self.model, 'fc_norm'):
            del self.model.fc_norm  # jeśli istnieje
