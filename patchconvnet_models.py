...
class Multi_Head_Latent_Attention(nn.Module):
    def __init__(
        ...
        self.low_rank_dim = low_rank_dim if low_rank_dim is not None else dim
        assert self.low_rank_dim % num_heads == 0, "low_rank_dim must be divisible by num_heads"
        self.q_head_dim = self.low_rank_dim // num_heads  
        self.head_dim = self.q_head_dim
        self.scale = qk_scale or self.head_dim ** -0.5
        self.q = nn.Linear(self.dim, self.dim, bias=qkv_bias)
        
        self.q = nn.Linear(dim, self.low_rank_dim, bias=qkv_bias)
        self.k = nn.Linear(dim, self.low_rank_dim, bias=qkv_bias)
        self.v = nn.Linear(dim, self.low_rank_dim, bias=qkv_bias)

            
        # latent projection (learnable latent vectors)
        self.latent_queries = nn.Parameter(torch.randn(num_classes, dim))
        nn.init.trunc_normal_(self.latent_queries, std=0.02)
        # self.out_dim = num_heads * self.kv_head_dim
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(self.num_heads * self.q_head_dim, dim) 
        self.proj_drop = nn.Dropout(proj_drop)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # latent queries -> Q
        q = self.q(self.latent_queries).unsqueeze(0).expand(B, -1, -1)  # [B, num_classes, C]
        q = q.reshape(B, self.num_classes, self.num_heads, self.q_head_dim).permute(0, 2, 1, 3)  # [B, H, cls, D]
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale  # [B, H, cls, N]
        attn = F.softmax(attn, dim=-1)
        attn = self.attn_drop(attn)
        
        x_cls = (attn @ v)  # [B, H, cls, D]
        # x_cls = x_cls.transpose(1, 2).reshape(B, self.num_classes, C)  # [B, cls, C]
        out_dim = self.num_heads * self.q_head_dim
        x_cls = x_cls.transpose(1, 2).reshape(B, self.num_classes, out_dim)
        x_cls = self.proj(x_cls)
        x_cls = self.proj_drop(x_cls)
...
@register_model
def S60_MLA(pretrained: bool = False, **kwargs):
    num_heads = kwargs.pop("num_heads", 8)
    depth_token_only = kwargs.pop("depth_token_only", 1)
    ...
    model = PatchConvnet(
        ...
        Patch_layer=ConvStem,
        Attention_block=Conv_blocks_se,
        Attention_block_token_only=partial(
            Multi_Head_Latent_Attention,
            num_classes=num_classes,
            low_rank_dim=None,  # ← jawnie brak compression
        ),
    ...
    return model
@register_model
def S60_MLA_LR(pretrained: bool = False, **kwargs):
    num_heads = kwargs.pop("num_heads", 8)
    depth_token_only = kwargs.pop("depth_token_only", 1)
    low_rank_dim = kwargs.pop("low_rank_dim", 32)
    num_classes = kwargs.pop("num_classes", 100)
    ...
    model = PatchConvnet(
        ...
        depth_token_only=depth_token_only,
        mlp_ratio_clstk=3.0,
        **kwargs  # teraz bez kolidujących argumentów
    )
